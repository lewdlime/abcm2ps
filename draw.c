				  ...
				  struct SYMBOL *s)	/* main note */
{
	...
	if (nb_only) {
		...
		yy = a * xm + b;
		if (s1->stem > 0) {
			ym = y_get(s1, 1, xm - 3, 6);
			if (ym > yy)
				b += ym - yy;
			b += 4;
		} else {
			ym = y_get(s1, 0, xm - 3, 6);
			if (ym < yy)
				b += ym - yy;
			b -= 12;
		}
...
/*fixme:should get the real width*/
			...
			y = y_get(p_voice->sym, 1, 0, realwidth);
			if (top < y)
				top = y;
			y = y_get(p_voice->sym, 0, 0, realwidth);
			if (bot > y)
				bot = y;
		}
